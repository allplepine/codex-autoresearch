...
def require_git_repo(start: Path | None = None) -> Path:
    ...
    if repo is None:
        raise AutoresearchError(
            "Managed repos must be git repositories so research-validation-skill can store repo-local pointers."
        )
    return repo
...
def artifact_root_from_start(start: Path | None = None) -> Path:
    ...
    if repo is not None:
        ...
        raise AutoresearchError(
            f"No research-validation-skill pointer is available for repo {repo}. "
...
def require_context_for_repo(repo: Path | None) -> CanonicalContext:
    ...
    try:
        ...
        raise AutoresearchError(
            f"Could not resolve research-validation-skill pointer path for repo {resolved_repo}: {exc}"
        ) from exc
    ...
    if pointer is None:
        ...
        raise AutoresearchError(
            f"No research-validation-skill context found for repo {resolved_repo}; expected repo-local "
