...
def build_runtime_prompt(
    *,
    ...
    lines = [
        ...
        f"Mode: {launch_manifest.get('mode', 'research')}",
        f"Idea: {config.get('goal', '')}",
    ...
    lines.extend(
        [
            ...
            f"- Check validation status: python3 {supervisor_script}",
            "",
            "Runtime checklist:",
            *[f"- {item}" for item in RUNTIME_CHECKLIST],
            ...
            "- When initializing fresh artifacts for this managed run, call autoresearch_init_run.py with --repo <primary_repo> and --workspace-root <workspace_root>.",
