...
1. Resolve the current run through the repo-local pointer and `generic-supervised-results/context.json`, then check `generic-supervised-results/state.json` first (primary recovery source), followed by `generic-supervised-results/results.tsv`, `generic-supervised-results/lessons.md`, and recent `experiment:` commits.
...
Do not create `generic-supervised-results/results.tsv` or `generic-supervised-results/state.json` before the baseline metric is known.
...
Here `<skill-root>` is the directory containing the loaded `SKILL.md`. In the common repo-local install this is usually `.agents/skills/generic-supervised-skill`, so the exact command becomes `python3 .agents/skills/generic-supervised-skill/scripts/...`.
...
5. Read `generic-supervised-results/lessons.md` if it exists (see `references/lessons-protocol.md`).
...
Consult `generic-supervised-results/lessons.md` (see `references/lessons-protocol.md`):
...
The results log (`generic-supervised-results/results.tsv`) serves as the true audit trail for all experiments, including discarded ones.
...
Do not hand-edit `generic-supervised-results/results.tsv` or `generic-supervised-results/state.json`.
...
On hard blocker, log the blocker reason in TSV with status `blocked` and stop. Keep the retained-state fields in `generic-supervised-results/state.json` unchanged (`current_metric`, `best_metric`, `best_iteration`, `last_commit`), but it is acceptable to advance audit counters such as `iteration`, `blocked`, `last_status`, and `last_trial_*` so the JSON snapshot stays aligned with the blocked TSV row. This preserves session resume without pretending the blocker improved the retained result.
