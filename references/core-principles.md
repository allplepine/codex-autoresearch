...
Kept experiments stay in history. Failed experiments are rolled back using the pre-approved experiment rollback strategy. The results log (`generic-supervised-results/results.tsv`) records every experiment -- kept or discarded -- as the true audit trail.
